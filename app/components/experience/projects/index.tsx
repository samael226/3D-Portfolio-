...
const Projects = () => {
  ...
  useFrame((state, delta) => {
    if (isActive) {
      if (!isMobile) {
        // Allow 360-degree rotation based on mouse movement
        const targetRotation = -(state.pointer.x * Math.PI * 2);
        camera.rotation.y = THREE.MathUtils.lerp(camera.rotation.y, targetRotation, 0.03);
